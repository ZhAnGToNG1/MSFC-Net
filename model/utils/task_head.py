...
class ori_head(nn.Module):
    def __init__(self , heads ,head_conv= 64):
        super(ori_head,self).__init__()
        self.heads = heads
        ...
        # p_offset(ori)
        self.poffset_64 = nn.Conv2d(256, head_conv, kernel_size=3, padding=1, bias=True)
        self.poffset_out = nn.Conv2d(head_conv, self.heads['poffset'], kernel_size=1, stride=1, padding=0)
    ...
    def init_weight(self):
        ...
        nn.init.kaiming_normal_(self.poffset_out.weight)
        nn.init.constant_(self.poffset_out.bias, 0)
    ...
    def forward(self, x):
        ...
        hm = self.hm_out(self.relu(self.hm_64(x)))
        ...
        wh = self.wh_out(self.relu(self.wh_64(x)))
        ret['wh'] = wh

        #point_offset regression
        poffset = self.poffset_out(self.relu(self.poffset_64(x)))
        ret['poffset'] = poffset

        # reg-regression

        reg = self.reg_out(self.relu(self.reg_64(x)))
        ret['reg'] = reg
